...
export function PermissionGate({
  ...
  if (workspaceId && organizationId && !permissions.canAccessWorkspace(workspaceId, organizationId)) {
    return <>{fallback}</>;
  }
...
export const CanManageOrganization: React.FC<{ children: React.ReactNode; fallback?: React.ReactNode }> = ({
  children,
  fallback,
}) => (
...
export const CanManageBilling: React.FC<{ children: React.ReactNode; fallback?: React.ReactNode }> = ({
  children,
  fallback,
}) => (
...
export const CanCreateWorkspaces: React.FC<{ children: React.ReactNode; fallback?: React.ReactNode }> = ({
  children,
  fallback,
}) => (
...
export const CanManageWorkspaces: React.FC<{ children: React.ReactNode; fallback?: React.ReactNode }> = ({
  children,
  fallback,
}) => (
...
export const CanExecuteWorkflows: React.FC<{ children: React.ReactNode; fallback?: React.ReactNode }> = ({
  children,
  fallback,
}) => (
...
export const CanShareProposals: React.FC<{ children: React.ReactNode; fallback?: React.ReactNode }> = ({
  children,
  fallback,
}) => (
...
export const CanAccessAdmin: React.FC<{ children: React.ReactNode; fallback?: React.ReactNode }> = ({
  children,
  fallback,
}) => (
...
export const CanManageContent: React.FC<{ children: React.ReactNode; fallback?: React.ReactNode }> = ({
  children,
  fallback,
}) => (
  <PermissionGate
...
export function RoleBadge({ role }: { role?: Role }) {
  ...
  return (
    <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium ${colors[role]}`}>
      {role}
    </span>
  );
}
...
export function PermissionDebugger() {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 max-w-xs rounded-lg bg-slate-800 p-4 text-xs text-white shadow-lg">
      <div className="font-semibold">Current User Permissions</div>
      <div className="mt-2">
        <div>Role: <RoleBadge role={permissions.user?.role} /></div>
        <div className="mt-2">
            ...
            {Object.entries(permissions)
              ...
              .map(([key, value]) => (
                <div key={key} className={`text-xs ${value ? 'text-green-400' : 'text-red-400'}`}>
                  {value ? '✓' : '✗'} {key.replace('can', '').replace(/([A-Z])/g, ' $1').toLowerCase()}
                </div>
              ))}
          </div>
        </div>
