  ...
  try {
    ...
    const workspaces = await Promise.all(
      workspacesSnapshot.docs.map(async (doc) => {
        ...
        const isMember =
          data.members?.some((member) => member.userId === context.user.uid) ||
 ...
 */
export const POST = withPermissions(['workspaces.create'], async (request, context) => {
  try {
 ...
 */
export const PUT = withPermissions(['workspaces.update'], async (request, context) => {
  try {
    const body = await request.json();
