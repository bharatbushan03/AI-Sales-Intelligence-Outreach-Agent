...
export async function POST(request: NextRequest, props: RouteParams) {
  return withPermissions(['workspaces.manage_members'], async (req, context) => {
    try {
      const params = await props.params;
      const workspaceId = params.id;
      ...
      if (workspaceData.organizationId !== context.organizationId) {
        return ApiResponse.permissionDenied(
          'Cannot add members to workspace in different organization',
        );
      }
...
export async function PUT(request: NextRequest, props: RouteParams) {
  return withPermissions(['workspaces.manage_members'], async (req, context) => {
    try {
      ...
      if (workspaceData.organizationId !== context.organizationId) {
        return ApiResponse.permissionDenied(
          'Cannot update members in workspace from different organization',
        );
      }
...
export async function DELETE(request: NextRequest, props: RouteParams) {
  return withPermissions(['workspaces.manage_members'], async (req, context) => {
    try {
      ...
      if (workspaceData.organizationId !== context.organizationId) {
        return ApiResponse.permissionDenied(
          'Cannot remove members from workspace in different organization',
        );
      }
      ...
      const updatedMembers =
        workspaceData.members?.filter((member) => member.userId !== userId) || [];
      await workspaceRef.update({
        members: updatedMembers,
        updatedAt: new Date().toISOString(),
      });
      ...
      if (userDoc.exists) {
        const userData = userDoc.data() as User;
        const updatedWorkspaceIds = (userData.workspaceIds || []).filter(
          (id) => id !== workspaceId,
        );
        ...
        await adminDb
          ...
          .update({
            ...
            'preferences.activeWorkspaceId':
              userData.preferences?.activeWorkspaceId === workspaceId
