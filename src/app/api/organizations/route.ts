 ...
 */
export const PUT = withPermissions(['organization.update'], async (request, context) => {
  try {
 ...
 */
export const DELETE = withPermissions(['organization.delete'], async (request, context) => {
  try {
    const { searchParams } = new URL(request.url);
