...
interface HallucinationReport {
  ...
  promptId: string;
  confidence: number;
  unsupportedClaims: Array<{ statement: string; evidence: string; confidence: number; sourceReference?: string }>;
...
export default function AIIntelligenceDashboard() {
  const [activeTab, setActiveTab] = useState<'registry' | 'analytics' | 'evaluations' | 'hallucinations' | 'cache'>('registry');
  ...
  useEffect(() => {
    if (comparePromptId && versionA !== '') {
      const matchA = versions.find(v => v.promptId === comparePromptId && v.version === Number(versionA));
      const matchB = versions.find(v => v.promptId === comparePromptId && v.version === Number(versionB));
      ...
      const aData = matchA 
        ? { template: matchA.template, systemInstruction: matchA.systemInstruction || '', version: matchA.version }
        : activePrompt && activePrompt.version === Number(versionA)
          ? { template: activePrompt.template, systemInstruction: activePrompt.systemInstruction || '', version: activePrompt.version }
          : undefined;

      const bData = matchB 
        ? { template: matchB.template, systemInstruction: matchB.systemInstruction || '', version: matchB.version }
    ...
    try {
      ...
      const res = await fetch('/api/intelligence', {
        ...
        body: JSON.stringify({
          ...
          changelog: editForm.changelog || `Updated template to version ${selectedPrompt.version + 1}`,
        }),
      });
  ...
  const totalTokens = tokenUsage.reduce((acc, curr) => acc + (curr.promptTokens || 0) + (curr.responseTokens || 0), 0);
  const averageLatency = tokenUsage.length > 0
    ? Math.round(tokenUsage.reduce((acc, curr) => acc + (curr.latencyMs || 0), 0) / tokenUsage.length)
    : 0;
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-400">
            Enterprise registry, structured output schema validations, self-critique metrics, A/B checks, and cost governance.
          </p>
        </div>
        <button
          onClick={fetchData}
          className="self-start rounded-xl border border-slate-800 bg-slate-900 hover:bg-slate-800 px-4 py-2 text-xs font-semibold text-slate-300 transition-all"
        >
      ...
      {loading ? (
        ...
        <div className="space-y-6">
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left p-4 rounded-xl border transition-all ${
                          isSelected
                            ? 'bg-slate-900/80 border-indigo-500/80 shadow-md shadow-indigo-600/5'
                            : 'bg-slate-900/30 border-slate-800 hover:bg-slate-900/50'
                        }`}
                      >
                        <div className="flex items-center justify-between">
                          <span className="text-sm font-semibold text-slate-200">{prompt.name}</span>
                          <span className="rounded-full bg-slate-800 px-2 py-0.5 text-[10px] text-slate-400 font-mono">
                            v{prompt.version}
                          </span>
                        </div>
                        <p className="mt-1 text-xs text-slate-400 line-clamp-1">{prompt.description}</p>
                  ...
                  <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6 space-y-6">
                    <div className="flex items-center justify-between border-b border-slate-800 pb-4">
                      <div>
                        ...
                        <span className="text-xs text-indigo-400 font-mono">{selectedPrompt.id}</span>
                      </div>
                      {!isEditing && (
                        <button
                          onClick={() => setIsEditing(true)}
                          className="flex items-center gap-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 px-3.5 py-1.5 text-xs font-semibold text-white transition-all"
                        >
                    ...
                    {isEditing ? (
                      <form onSubmit={handleSavePrompt} className="space-y-4">
                        ...
                        <div>
                          ...
                          <textarea
                            ...
                            rows={4}
                            className="w-full rounded-lg border border-slate-700 bg-slate-900/60 px-3.5 py-2 text-xs font-mono text-indigo-300 focus:border-indigo-500 focus:outline-none"
                          />
                        </div>
                        ...
                        <div>
                          ...
                          <textarea
                            ...
                            rows={4}
                            className="w-full rounded-lg border border-slate-700 bg-slate-900/60 px-3.5 py-2 text-xs font-mono text-slate-200 focus:border-indigo-500 focus:outline-none"
                            required
                          />
                        </div>

                        <div>
                          <label className="block text-xs font-medium text-slate-400 mb-1">Few Shots Examples (one per line)</label>
                          <textarea
                            value={editForm.fewShots}
                            onChange={e => setEditForm({ ...editForm, fewShots: e.target.value })}
                            rows={3}
                            placeholder="Input: ... \nOutput: ..."
                            className="w-full rounded-lg border border-slate-700 bg-slate-900/60 px-3.5 py-2 text-xs font-mono text-slate-300 focus:border-indigo-500 focus:outline-none"
                          />
                        </div>

                        <div>
                          <label className="block text-xs font-medium text-slate-400 mb-1">Output Schema Instructions</label>
                          <input
                            type="text"
                            value={editForm.outputInstructions}
                            onChange={e => setEditForm({ ...editForm, outputInstructions: e.target.value })}
                        ...
                        <div>
                          <label className="block text-xs font-medium text-indigo-400 mb-1">Changelog (Required to bump version)</label>
                          <input
                            type="text"
                        ...
                        <div className="flex justify-end gap-3 pt-2">
                          <button
                            ...
                            className="rounded-lg border border-slate-700 hover:bg-slate-800 px-4 py-2 text-xs font-semibold text-slate-300"
                          >
                            Cancel
                          </button>
                          <button
                            type="submit"
                            disabled={saving}
                            className="rounded-lg bg-indigo-600 hover:bg-indigo-500 disabled:bg-indigo-800 px-4 py-2 text-xs font-semibold text-white"
                          >
                      ...
                      <div className="space-y-5">
                        <div>
                          ...
                          <p className="mt-1 text-sm text-slate-300">{selectedPrompt.description}</p>
                        </div>

                        {selectedPrompt.systemInstruction && (
                          <div>
                            <span className="text-xs font-semibold text-slate-400 uppercase">System Instruction</span>
                            <pre className="mt-1.5 p-3 rounded-lg border border-slate-800 bg-slate-950/50 overflow-x-auto text-[11px] font-mono text-indigo-300 whitespace-pre-wrap">
                              {selectedPrompt.systemInstruction}
                            </pre>
                          </div>
                        )}
                        ...
                        <div>
                          <span className="text-xs font-semibold text-slate-400 uppercase">Active Template</span>
                          <pre className="mt-1.5 p-3 rounded-lg border border-slate-800 bg-slate-950/50 overflow-x-auto text-[11px] font-mono text-emerald-300 whitespace-pre-wrap">
                        ...
                        {selectedPrompt.fewShots && selectedPrompt.fewShots.length > 0 && (
                          <div>
                            <span className="text-xs font-semibold text-slate-400 uppercase">Few Shot Examples</span>
                            <div className="mt-1.5 space-y-2">
                              {selectedPrompt.fewShots.map((shot, i) => (
                                <pre key={i} className="p-3 rounded-lg border border-slate-800 bg-slate-950/30 overflow-x-auto text-[11px] font-mono text-slate-400 whitespace-pre-wrap">
                                  {shot}
                                </pre>
                              ))}
                            </div>
                        ...
                        <div className="border-t border-slate-800 pt-6 space-y-4">
                          <div className="flex items-center gap-2 text-sm font-semibold text-slate-200">
                            <GitCompare className="h-4 w-4 text-indigo-400" /> A/B Version Comparison Diff
                          </div>
                          <div className="flex flex-wrap gap-4 items-end">
                            <div>
                              <label className="block text-[10px] text-slate-400 mb-1">Select Version A</label>
                              <select
                                value={versionA}
                              ...
                              >
                                ...
                                <option value={selectedPrompt.version}>v{selectedPrompt.version} (Active)</option>
                              </select>
                            </div>
                            <div>
                              <label className="block text-[10px] text-slate-400 mb-1">Select Version B</label>
                              <select
                                value={versionB}
                          ...
                          {compareResult && compareResult.promptA && compareResult.promptB && (
                            <div className="grid gap-4 md:grid-cols-2">
                              <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-4 space-y-2">
                                ...
                                <div className="text-[11px] font-mono text-slate-300 whitespace-pre-wrap bg-slate-950 p-2.5 rounded-lg max-h-48 overflow-y-auto border border-slate-800">
                        ...
                        <div className="border-t border-slate-800 pt-6 space-y-3">
                          ...
                          <div className="divide-y divide-slate-800/80">
                            {versions
                              .filter(v => v.promptId === selectedPrompt.id)
                              .map(v => (
                                <div key={v.id} className="py-2.5 flex items-start justify-between text-xs">
                                  <div>
                                    <span className="font-semibold text-slate-200 font-mono">v{v.version}</span>
                                    <p className="text-slate-400 mt-0.5">{v.changelog}</p>
                                  </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
                <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-6">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold tracking-wider text-indigo-400 uppercase">Estimated Total Cost</span>
                    <Coins className="h-5 w-5 text-indigo-400" />
                  </div>
                  <div className="mt-4">
                    <span className="text-3xl font-bold text-white">${totalCost.toFixed(4)}</span>
                    <p className="mt-1 text-[10px] text-slate-500">Based on prompt & response tokens</p>
                  </div>
                </div>

                <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-6">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold tracking-wider text-emerald-400 uppercase">Total Tokens Consumed</span>
                    <Cpu className="h-5 w-5 text-emerald-400" />
                  </div>
                  <div className="mt-4">
                    <span className="text-3xl font-bold text-white">{totalTokens.toLocaleString()}</span>
                    <p className="mt-1 text-[10px] text-slate-500">Cumulative prompt + completion</p>
                  </div>
                </div>

                <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-6">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold tracking-wider text-amber-400 uppercase">Average Latency</span>
                ...
                <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-6">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold tracking-wider text-rose-400 uppercase">Platform Executions</span>
              ...
              <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6">
                <h2 className="text-lg font-semibold text-slate-200 mb-4">Prompt Execution & Governance Logs</h2>
                  ...
                  <table className="w-full text-left text-xs">
                    <thead>
                      <tr className="border-b border-slate-800 text-slate-400 font-semibold uppercase tracking-wider">
                        ...
                        <th className="pb-3 px-4 font-mono">Prompt ID</th>
                        <th className="pb-3 px-4 text-right">Tokens</th>
                        <th className="pb-3 px-4 text-right">Latency</th>
          ...
          {activeTab === 'evaluations' && (
            <div className="space-y-6">
              ...
              <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6 space-y-4">
                <h2 className="text-lg font-semibold text-slate-200">Agent Quality Scorecard Matrix</h2>
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
                  {['ResearchAgent', 'OpportunityAgent', 'OutreachAgent', 'CrmAgent', 'ProposalAgent'].map(agentName => {
                    ...
                    return (
                      <div key={agentName} className="rounded-xl border border-slate-800 bg-slate-950 p-5 space-y-3">
                        <span className="text-xs font-semibold text-slate-200">{agentName}</span>
                        <div className="flex items-baseline justify-between">
                          <span className="text-2xl font-bold text-white">{displayQuality}/100</span>
                          <span className="text-[10px] text-emerald-400 font-semibold">{displaySuccess}% SR</span>
                        </div>
                        ...
                        <div className="w-full bg-slate-850 h-1.5 rounded-full overflow-hidden">
                          <div 
                            className={`h-full rounded-full ${
                              displayQuality >= 80 ? 'bg-emerald-500' : displayQuality >= 60 ? 'bg-amber-500' : 'bg-rose-500'
                            }`}
              ...
              <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6">
                <h2 className="text-lg font-semibold text-slate-200 mb-4">Live Evaluation Audit Trail</h2>
                <div className="space-y-4">
                  {evaluations.map(result => (
                    <div key={result.id} className="rounded-xl border border-slate-800 bg-slate-950 p-5 space-y-4">
                      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-900 pb-3">
                        <div className="flex items-center gap-3">
                          <span className="rounded-lg bg-indigo-950 border border-indigo-900 px-2.5 py-1 text-xs font-semibold text-indigo-400">
                            {result.agentName}
                          </span>
                          <span className="text-xs text-slate-400 font-mono">{result.promptId}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-xs text-slate-500">Overall Score:</span>
                          <span className="text-sm font-bold text-emerald-400">{result.overallScore} / 100</span>
                        </div>
                      </div>
                      ...
                      <div className="grid gap-4 grid-cols-2 sm:grid-cols-3 lg:grid-cols-6">
                        {[
                          ...
                          <div key={metric.label} className="bg-slate-900/50 p-2.5 rounded-lg border border-slate-900 text-center">
                            <span className="block text-[9px] text-slate-500 uppercase tracking-wider font-semibold">{metric.label}</span>
                            <span className="text-sm font-bold text-slate-200 mt-1 block">{metric.value}</span>
                          </div>
                        ))}
                      </div>
                      ...
                      {result.critique && (
                        <div className="text-xs bg-slate-900/30 p-3 rounded-lg border border-slate-900">
                          <span className="block font-semibold text-indigo-400 mb-1">Self-Critique Review</span>
                          <p className="text-slate-300 leading-relaxed">{result.critique}</p>
                        </div>
                      )}

                      {result.improvements && result.improvements.length > 0 && (
                        <div className="text-xs">
                          <span className="block font-semibold text-emerald-400 mb-1.5">Actionable Self-Correction Improvements</span>
          ...
          {activeTab === 'hallucinations' && (
            <div className="space-y-6">
              <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6">
                <h2 className="text-lg font-semibold text-slate-200 mb-4">Hallucination & Grounding Reports</h2>
                <div className="space-y-4">
                  {hallucinations.map(rep => {
                    const isPassed = rep.status === 'passed';
                    const isWarning = rep.status === 'warning';
                    return (
                      <div key={rep.id} className={`rounded-xl border p-5 space-y-4 ${
                        isPassed 
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-900/60 pb-3">
                          <div className="flex items-center gap-3">
                            <span className="font-mono text-xs text-indigo-400">{rep.promptId}</span>
                          ...
                          <div className="flex items-center gap-2">
                            {isPassed ? (
                              <span className="flex items-center gap-1 text-xs font-semibold text-emerald-400 bg-emerald-950/50 px-2.5 py-0.5 rounded-full border border-emerald-900/50">
                                <CheckCircle className="h-3 w-3" /> Grounded & Verified
                              </span>
                            ) : (
                              <span className={`flex items-center gap-1 text-xs font-semibold px-2.5 py-0.5 rounded-full border ${
                                isWarning 
                        ...
                        {rep.unsupportedClaims && rep.unsupportedClaims.length > 0 ? (
                          <div className="space-y-3">
                            <span className="block text-xs font-semibold text-slate-400">Claims Grounding Check:</span>
                            <div className="space-y-2">
                              {rep.unsupportedClaims.map((claim, idx) => (
                                <div key={idx} className="rounded-lg bg-slate-900/40 border border-slate-900 p-3 text-xs space-y-2">
                                  <div className="flex items-start justify-between gap-2">
                                    <p className="text-slate-200 font-medium">"{claim.statement}"</p>
                                    <span className={`shrink-0 font-semibold px-2 py-0.5 rounded text-[9px] ${
                                  ...
                                  <div className="grid gap-2 sm:grid-cols-2 text-[11px]">
                                    <div>
                                      <span className="block text-slate-500 font-semibold">Evidence</span>
                                      <p className="text-slate-400 mt-0.5">{claim.evidence}</p>
                                    </div>
                                    {claim.sourceReference && (
                                      <div>
                                        <span className="block text-slate-500 font-semibold">Reference Source</span>
                                        <p className="text-slate-400 mt-0.5 truncate">{claim.sourceReference}</p>
                                      </div>
                                    )}
                                  </div>
                                </div>
                          ...
                          <p className="text-xs text-slate-400">
                            Zero unsupported claims or anomalies identified. Response is perfectly grounded in retrieved data.
                          </p>
                        )}

                        {rep.contradictions && rep.contradictions.length > 0 && (
                          <div className="text-xs space-y-1.5 border-t border-slate-900/60 pt-3">
                            <span className="block font-semibold text-red-400">Logical Contradictions Identified:</span>
                  ...
                  {hallucinations.length === 0 && (
                    <div className="py-8 text-center text-slate-500 text-sm">
                      No hallucination warnings recorded. Agent outputs are currently fully grounded.
                    </div>
                  )}
                </div>
              </div>
          ...
          {activeTab === 'cache' && (
            <div className="space-y-6">
              ...
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-6">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold tracking-wider text-indigo-400 uppercase">Cached Query Outputs</span>
                    <Database className="h-5 w-5 text-indigo-400" />
                  </div>
                  <div className="mt-4">
                    <span className="text-3xl font-bold text-white">{cacheEntries.length}</span>
                    <p className="mt-1 text-[10px] text-slate-500">Currently stored active schemas</p>
                  </div>
                </div>

                <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-6">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold tracking-wider text-emerald-400 uppercase">Average Cache Speed</span>
                ...
                <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-6">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold tracking-wider text-amber-400 uppercase">Estimated Savings</span>
                  ...
                  <div className="mt-4">
                    ...
                    <p className="mt-1 text-[10px] text-slate-500">Saved by bypassing redundant generations</p>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6">
                <div className="flex items-center gap-2 mb-4">
                  <Database className="h-5 w-5 text-indigo-400" />
                  <h2 className="text-lg font-semibold text-slate-200">Active Gemini Response Cache Map</h2>
                </div>
                <div className="space-y-3">
                  {cacheEntries.map(entry => (
                    <div key={entry.id} className="rounded-xl border border-slate-800 bg-slate-950 p-4 space-y-2">
                      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-900 pb-2">
                        <div className="flex items-center gap-2.5">
                          ...
                          <span className="font-mono text-xs text-indigo-400">{entry.promptId}</span>
                        </div>
                        <span className="text-[10px] text-slate-500">
                      ...
                      <div className="text-xs">
                        <span className="block text-slate-500 font-semibold">Cached Output Text Snippet</span>
                        <pre className="mt-1 p-2 bg-slate-900/30 rounded border border-slate-900/80 font-mono text-[10px] text-slate-400 max-h-16 overflow-y-auto truncate">
                          {entry.text}
                        </pre>
                      </div>
                    </div>
